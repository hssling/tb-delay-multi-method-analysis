"""Ingest public TB datasets and export cleaned CSVs - Version 2 with updated data sources."""
...
LOG_PATH = PROCESSED_DIR / "ingest_sources_v2.log"
...
DATASETS = {
    ...
    "tb_prevalence": RAW_DIR / "tb_prevalence_survey.xlsx",  # CSV content, but named .xlsx
    "nfhs": RAW_DIR / "nfhs_state_indicators.csv",
    "census": RAW_DIR / "census_state_indicators.xlsx",  # Updated to Excel
}


def configure_logging() -> logging.Logger:
    logger = logging.getLogger("ingest_sources_v2")
    if logger.handlers:
        return logger
...
def save_dataset(df: pd.DataFrame, name: str, logger: logging.Logger) -> None:
    ...
    output_path = PROCESSED_DIR / f"{name}_clean_v2.csv"
...
def ingest_datasets(selected: Dict[str, Path], logger: logging.Logger) -> None:
    loaders: Dict[str, Callable[[Path], pd.DataFrame]] = {
        ...
        "tb_prevalence": load_csv,  # CSV content
        "nfhs": load_csv,
        "census": load_excel,  # Updated
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest WHO/NFHS/Census tuberculosis datasets - Version 2."
    )
    parser.add_argument(
        "--skip-who",
        action="store_true",
...
def main() -> None:
    ...
    logger.info("Starting ingestion of TB data sources - Version 2.")
    ...
    logger.info("Ingestion complete - Version 2.")
